...
skill_paths.each do |path|
  ...
  if text.include?("/private/skill-source")
    failures << "#{relative_path}: contains a corporate absolute skill path"
  end

  if text.include?("private-repository")
...
if File.file?(coverage_map)
  ...
  private_collaboration_hosts = ["private-collaboration-host", "private-collaboration-host"].freeze
