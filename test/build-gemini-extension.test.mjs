...
test('keeps the previous generated bundle when staging fails', () => {
  const parent = fs.mkdtempSync(path.join(os.tmpdir(), 'fulcrum-gemini-stage-'));
  const destination = path.join(parent, 'fulcrum-ai-toolkit-gemini');
  const marker = path.join(destination, '.fulcrum-ai-toolkit-gemini-generated');
  const license = path.join(destination, 'LICENSE');

  try {
    assembleGeminiExtension(destination);
    const previousLicense = fs.readFileSync(license, 'utf8');
    const previousMarker = fs.readFileSync(marker, 'utf8');

    fs.chmodSync(parent, 0o500);
    try {
      assert.throws(() => assembleGeminiExtension(destination), /EACCES|permission denied/i);
    } finally {
      fs.chmodSync(parent, 0o700);
    }

    assert.equal(fs.readFileSync(license, 'utf8'), previousLicense);
...
test('rejects symlinks in the Gemini source tree before copying', () => {
  ...
  const sourceTarget = path.join(parent, 'source-target');
