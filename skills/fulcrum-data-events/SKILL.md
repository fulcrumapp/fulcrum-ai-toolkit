...
`STORAGE()` returns a local-storage-like object with `getItem`, `setItem`, `removeItem`, and `clear` methods. Values must be strings, so serialize objects with `JSON.stringify()`. The store is device-wide and persistent, so a bare key such as `baseline` is still there when the next record opens. Scope saved-record keys with `FORM().id`, the record it belongs to, and the editing session's nonce so concurrent editors never share a key; remove that session key on `cancel-record` and `unload-record`. `RECORDID()` is null until a new record has been saved, so it cannot separate one unsaved record from the next on its own. For unsaved records, keep the baseline in the editing session's memory rather than creating persistent draft keys that can accumulate after a crash. This example assumes lifecycle callbacks for one editing session share the same script context; if a host can interrupt callbacks, provide an ephemeral session store or a retention policy for stale saved-record keys. See
