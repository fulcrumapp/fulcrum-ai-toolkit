...
`STORAGE()` returns a local-storage-like object with `getItem`, `setItem`, `removeItem`, and `clear` methods. Values must be strings, so serialize objects with `JSON.stringify()`. The store is device-wide and persistent, so a bare key such as `baseline` is still there when the next record opens. Scope saved-record keys with `FORM().id` and the record it belongs to, stamp stored values with the editing session's nonce, and remove them on `cancel-record` and `unload-record` only when the current session still owns them. `RECORDID()` is null until a new record has been saved, so it cannot separate one unsaved record from the next on its own. For unsaved records, keep the baseline in the editing session's memory rather than creating persistent draft keys that can accumulate after a crash. This example assumes lifecycle callbacks for one editing session share the same script context; if a host recreates that context, use a host-specific ephemeral session mechanism instead of a persistent draft key without a retention policy. See
