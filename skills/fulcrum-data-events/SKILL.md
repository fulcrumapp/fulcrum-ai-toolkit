...
`STORAGE()` returns a local-storage-like object with `getItem`, `setItem`, `removeItem`, and `clear` methods. Values must be strings, so serialize objects with `JSON.stringify()`. The store is device-wide and persistent, so it is not safe for an edit-session baseline: concurrent editors can race and interrupted callbacks can leave orphaned keys. Keep this example's baseline in the editing session's memory instead. `RECORDID()` is null until a new record has been saved, but even a saved record identifier cannot scope state to one concurrent editing session. This example assumes lifecycle callbacks for one editing session share the same script context; if a host recreates that context, use a host-specific ephemeral session mechanism instead of persistent storage. See
