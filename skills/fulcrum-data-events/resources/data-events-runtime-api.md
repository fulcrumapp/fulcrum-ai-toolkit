...
8. STORAGE() is device-wide and persistent — scope every key with `FORM().id` and a saved `RECORDID()`, bound the payload, and remove it on `cancel-record` and `unload-record`. The storage-session example expires persistent entries after 30 minutes without a context reload; STORAGE() has no session identifier, so a new edit within that window cannot be distinguished from context recreation. Use a host-specific session store for exact concurrent-session isolation. The example persists baselines for records that already have an identifier; a newly created record remains in ordinary script state because `save-record` runs before its identifier is available for re-keying.
