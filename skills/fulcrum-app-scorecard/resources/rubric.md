...
| OUTPUT-02 | Deliverable | Representative output was checked for correctness, missing/empty data, and larger datasets. Evaluate query fan-out, join growth, and output-generation cost for the expected workload; for reports include rendering, media, tables, and pagination; for datasets include schema/values and joins. Separate static cost assessment from measured timings. A template that merely parses is not runtime evidence. | Never; absent output evidence is unknown |
...
Use [performance review](../../fulcrum-performance-review/SKILL.md) for every
code artifact. Missing workload/code evidence is unknown; a demonstrated
