...
- `record-link-field.json` is a complete element, not a fragment. Every
  ...
  `linked_form_id` or `allow_empty_records` element attribute. Rails accepts
  those unknown keys on input and drops them; responses serialize only
  declared properties. Rails stores the target form's database id in
  `forms_links.linked_form_id`; that association column is not part of the
  element.
