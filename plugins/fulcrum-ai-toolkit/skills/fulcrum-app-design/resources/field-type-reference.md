...
| record_conditions | array or null | Filters on the linked form. A serialized item has `linked_form_field_key`, `operator`, and either `value` or `value_field_key`. Missing keys are stored as null. Unknown keys are dropped. If both `value` and `value_field_key` are sent, only `value_field_key` is returned. `value` keeps its JSON type. |
| record_defaults | array or null | Values copied onto a newly created linked record. A serialized item has `source_field_key` and `destination_field_key`. Missing keys are stored as null. Unknown keys are dropped. |
