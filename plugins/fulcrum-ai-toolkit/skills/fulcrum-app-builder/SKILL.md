...
When a connector response includes a safe public `trace_id`, include it in the handoff as **Trace ID** and tell the user to provide it to support. Prefer that structured value; if only HTTP headers are exposed, extract a valid 32-hex-character trace ID from the `traceparent` value's second field, or a valid 16- or 32-hex-character trace ID from the B3 `b3` value's first field or `X-B3-TraceId` header. Do not use a sampling flag, span ID, or `tracestate` as the trace ID, and do not invent one when no valid value is supplied.
