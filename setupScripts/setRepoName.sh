...
# Handle REPO_TYPE to update outputType in ci/config.yaml and manage skaffold file
...
# Handle skaffold.yaml for npm packages
if [ "$REPO_TYPE" = "npm" ]; then
    echo "Repository type is npm, removing skaffold.yaml as it's not needed for npm packages"
    if [ -f "skaffold.yaml" ]; then
        echo "Removing skaffold.yaml file"
        rm -f skaffold.yaml
        git add skaffold.yaml 2>/dev/null || true
    else
        echo "skaffold.yaml does not exist, nothing to remove"
    fi
else
    echo "Repository type is $REPO_TYPE, keeping skaffold.yaml"
fi
...
echo "DEBUG: ENABLE_HELM value is: '$ENABLE_HELM'"

if [ "$ENABLE_HELM" != "TRUE" ]; then
    echo "ENABLE_HELM is not TRUE ($ENABLE_HELM), removing Helm configurations..."
    ...
    if [ -f "ci/config.yaml" ]; then
        ...
        # Use sed as a fallback if yq is not available
        if command -v yq >/dev/null 2>&1; then
            yq eval '.charts = []' ci/config.yaml > ci/config.yaml.tmp && mv ci/config.yaml.tmp ci/config.yaml
        else
            echo "yq not available, using sed to remove charts section"
            # Remove the entire charts section and replace with empty array
            sed -i='' '/^charts:/,/^[a-zA-Z]/ { /^charts:/ { s/.*/charts: []/; b }; /^[a-zA-Z]/ !d }' ci/config.yaml
        fi
    fi
    ...
    if [ -f "skaffold.yaml" ]; then
        ...
        if command -v yq >/dev/null 2>&1; then
            ...
            echo "yq not available, using sed to remove deploy section"
            # Remove the deploy section from skaffold.yaml
            sed -i='' '/^deploy:/,/^[a-zA-Z]/ { /^deploy:/ d; /^[a-zA-Z]/ !d }' skaffold.yaml
        fi
    fi
